...
import { renderFinanceReportPdf } from '../lib/pdf/financeReport.js'
...
reportsRouter.get('/period.pdf', async (req: Request, res: Response) => {
  ...
  const pdf = await renderFinanceReportPdf(report)

  const filename = `statement-${period.from}-to-${period.to}.pdf`

  res.setHeader('Content-Type', 'application/pdf')
  res.setHeader('Content-Disposition', `attachment; filename="${filename}"`)
  // A financial statement must never be served from a cache: the figures change
  // as entries are approved.
