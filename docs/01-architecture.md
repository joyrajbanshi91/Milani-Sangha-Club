...
Appwrite Sites serves the built PWA, and the same Express app runs as an Appwrite
Function on its own domain — so the browser talks to two origins
and needs no CORS exception in production. This works on the free tier, with no
card. See [10-appwrite.md](10-appwrite.md).
